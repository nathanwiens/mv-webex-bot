...
def getmvzones(apikey, serialnumber, suppressprint=False):

    calltype = 'MV Sense Zones'
    geturl = '{0}/devices/{1}/camera/analytics/zones'.format(str(base_url), str(serialnumber))
    headers = {
        'x-cisco-meraki-api-key': format(str(apikey)),
        'Content-Type': 'application/json'
    }
...
def updateswitchport(apikey, serialnum, portnum, name=None, tags=None,
    ...
    if stpguard and stpguard not in ['disabled', 'loop guard', 'root guard', 'BPDU guard']:
